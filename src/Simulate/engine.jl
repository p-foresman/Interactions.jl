...
######################## game algorithm ####################
function run_period!(state::State)
...
function play_game!(state::State)
...
function calculate_expected_utilities!(state::State)
    @inbounds for column in axes(payoff_matrix(model), 2) #column strategies #NOTE: could just do 1:size(model, dim=2) or something. might be a bit faster
        for row in axes(payoff_matrix(state.model), 1) #row strategies
            Interactions.increment_expected_utilities!(state, 1, row, payoff_matrix(state.model)[row, column][1] * Interactions.opponent_strategy_probabilities(state, 1, column))
            Interactions.increment_expected_utilities!(state, 2, column, Interactions.payoff_matrix(state.model)[row, column][2] * Interactions.opponent_strategy_probabilities(state, 2, row))
        end
    end
    return nothing
end
...
function make_choices!(state::State)
    for player_number in 1:2 #eachindex(model.pre_allocated_arrays.players)
        ...
        Interactions.choice!(Interactions.players(state, player_number), rand() < Interactions.error_rate(state.model) ? Interactions.random_strategy(state.model, player_number) : Interactions.rational_choice(Interactions.players(state, player_number)))
    end
end
...
function push_memories!(state::State)
    push_memory!(Interactions.players(state, 1), Interactions.choice(Interactions.players(state, 2)), memory_length(state.model))
    push_memory!(Interactions.players(state, 2), Interactions.choice(Interactions.players(state, 1)), memory_length(state.model))
    return nothing
end
